...
<form method="post" action="{{ route('roles.update', $role->id) }}" class="space-y-8" enctype="multipart/form-data">
@method('PUT')
@csrf
...
<div class="sm:col-span-3">
    ...
    <div class="mt-1">
    <input value="{{ $role->name }}" type="text" name="name" id="name" autocomplete="given-name" class="shadow-sm focus:ring-isptec focus:border-isptec block w-full sm:text-sm border-gray-300 rounded-md">
    </div>
    @if($errors->has('name'))
...
<div class="sm:col-span-3">
    ...
    <div class="mt-1">
    <input value="{{ $role->label }}" type="text" name="label" id="label" autocomplete="given-label" class="shadow-sm focus:ring-isptec focus:border-isptec block w-full sm:text-sm border-gray-300 rounded-md">
    </div>
    @if($errors->has('label'))
